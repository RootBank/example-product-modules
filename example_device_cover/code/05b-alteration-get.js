...
const getAlteration = ({ alteration_hook_key, data, policy, policyholder }) => {
  ...
  console.log('policy.module:', policy.module);
  console.log('data:', data);

  // Build up change message and new quote data before using it to get a new quote
  switch (alteration_hook_key) {
    case "change_cover":
      changeDescription = `cover changed to ${data.cover_type} with loaner device ${data.loaner_device ? '' : 'not'} included, excess set to ${data.excess}, claims history to ${data.claims_history} and area code to ${data.area_code}.`;
    ...
    case "change_devices":
      changeDescription = `devices updated to a new list.`;
      quoteData = { ...quoteData, devices: data.devices };
      break;
  ...
  console.log('quotePackage:', JSON.stringify(quotePackage));
