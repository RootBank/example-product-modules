...
const validateAlterationPackageRequest = ({ alteration_hook_key, data, policy, policyholder }) => {
  switch (alteration_hook_key) {
    case 'amendment':
      const result = validateChangePetCover({ data, policy, policyholder });
      return result;
    default:
...
const getAlteration = ({ alteration_hook_key, data, policy }) => {
  switch (alteration_hook_key) {
    case 'amendment':
      const changeDescription = updatedCoverAlterationMessage(data.pets, policy.module.pets);
      // Rebuild Policy Module with updated excess data
      const updatedPolicyModule = updatePetCover(data, policy.module);
      // Get updated quote
